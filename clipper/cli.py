...
def build_common_parent() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--store",
        default=os.environ.get("CLIPPER_STORE_PATH", ".clipper"),
...
def emit_result(config: CommandConfig, command: str, message: str) -> None:
    ...
    if config.json_output:
        print(json.dumps({"ok": True, "result": {"command": command, "message": message}}, separators=(",", ":")))
    else:
        print(message)
...
def add_placeholder_subcommands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser], common: argparse.ArgumentParser) -> None:
    ...
    start.add_argument("--reuse", action="store_true", help="Reuse existing artifacts when valid.")
    start.add_argument("--force", action="store_true", help="Overwrite target artifacts.")
    ...
    transcribe.add_argument("--reuse", action="store_true", help="Reuse existing transcript when valid.")
    transcribe.add_argument("--force", action="store_true", help="Overwrite transcript output.")
    ...
    score.add_argument("--reuse", action="store_true", help="Reuse existing scores when valid.")
    score.add_argument("--force", action="store_true", help="Overwrite scores output.")
    ...
    cut.add_argument("--reuse", action="store_true", help="Reuse existing clips when valid.")
    cut.add_argument("--force", action="store_true", help="Overwrite clip outputs.")
    ...
    montage.add_argument("--reuse", action="store_true", help="Reuse existing montage when valid.")
    montage.add_argument("--force", action="store_true", help="Overwrite montage outputs.")
    ...
    pipeline.add_argument("--reuse", action="store_true", help="Reuse existing artifacts when valid.")
    pipeline.add_argument("--force", action="store_true", help="Overwrite target artifacts.")
